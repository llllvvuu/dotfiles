...
return {
  ...
  config = function(_, opts)
    ...
    local suggestions = require("copilot.suggestion")
      ...
      map("i", "‘", suggestions.next, { desc = "[copilot] next", silent = true })
      map("i", "<M-‘>", suggestions.next, { desc = "[copilot] next", silent = true })

      map("i", "“", suggestions.prev, { desc = "[copilot] prev", silent = true })
      map("i", "<M-“>", suggestions.prev, { desc = "[copilot] prev", silent = true })
    -- stylua: ignore end
  end,
}
